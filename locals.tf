...
locals {
  ...
  ai_search_id     = var.ai_search_create ? module.ai_search[0].resource_id : var.ai_search.id
  ai_search_parsed = provider::azurerm::parse_resource_id(local.ai_search_id)

  # # AI Services

  ai_service_id     = var.ai_service_create ? module.ai_service[0].resource_id : var.ai_service.id
