...
module "orchestrator_function_app" {
  ...
  storage_account_name          = local.orchestrator_storage_account_parsed.resource_name
  storage_uses_managed_identity = true
  managed_identities            = try(var.orchestrator_function_app.managed_identities, { system_assigned = true })
  tags                          = merge(var.tags, try(var.orchestrator_function_app.tags, null))
...
module "data_ingestion_function_app" {
  ...
  location                      = var.location
  service_plan_resource_id      = local.app_service_plan_id
  storage_account_name          = local.data_ingestion_storage_account_parsed.resource_name
  kind                          = "functionapp"
  os_type                       = "Linux"
  enable_application_insights   = false
  https_only                    = true
  public_network_access_enabled = !var.use_private_networking
  virtual_network_subnet_id     = var.use_private_networking ? var.app_services_subnet_id : null
  managed_identities            = try(var.data_ingestion_function_app.managed_identities, { system_assigned = true })
  tags                          = var.tags
  enable_telemetry              = var.enable_telemetry
